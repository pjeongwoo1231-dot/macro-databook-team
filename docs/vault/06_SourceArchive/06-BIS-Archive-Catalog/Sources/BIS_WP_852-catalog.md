---
title: "BIS WP 852 — Average inflation targeting and the interest rate lower bound"
type: primary_source
duplicate_of_deep_note: "BIS_WP_852"
institution: "Bank for International Settlements"
series: "BIS Working Papers"
number: 852
published: "April 2020"
authors: "Flora Budianto , Taisuke Nakata and Sebastian Schmidt"
source_kind: "working-paper"
peer_reviewed: false
primary_text_available: true
fulltext_verified: true
pages: 29
text_characters: 77649
sha256: "9b88009b43732cc11bc39938779b5d4b66be1b04a1b188ed3b9ef54a65820015"
catalog_filename: "BIS_WP_852-catalog.md"
human_verified: false
catalog_method: "deterministic-keyword-index"
created: 2026-08-14
updated: 2026-08-14
archive_status: "official-fulltext-catalogued"
tags:
  - flag/partial-check
  - bis
  - working-paper
  - monetary-policy-and-central-banking
  - inflation-and-prices
  - macro-modelling-forecasting-and-data
status: working
verification: partial
reliability: working-paper
verified: "△ 공식 PDF에서 추출한 초록·서지·SHA-256. 원문 파일 미동봉이라 볼트에서 재대조 불가 — 초록 범위 밖 주장은 원문 확인"
related: ["[[원문 아카이브 MOC]]"]
text_basis: extracted-abstract
vault_tier: D
---

# BIS WP 852 — Average inflation targeting and the interest rate lower bound

> 이 파일은 공식 PDF 전문을 로컬 아카이브에 확보하고, 재현 가능한 키워드 규칙으로 색인한 **원문 카탈로그 노트**입니다. 분석적 요약 노트가 아니며, 저자의 주장·방법·한계를 인용하려면 원문 전문을 직접 확인해야 합니다.

| 항목 | 내용 |
|---|---|
| 저자 | Flora Budianto , Taisuke Nakata and Sebastian Schmidt |
| 발행 | April 2020 |
| 공식 PDF | [전문](https://www.bis.org/publ/work852.pdf) |
| 공식 랜딩페이지 | [페이지](https://www.bis.org/publ/work852.htm) |
| 검증 | PDF 서명·페이지 수·텍스트 추출·SHA-256 확인 |
| 페이지·텍스트 | 29쪽 · 77,649자 |
| 자동 주제색인 | monetary-policy-and-central-banking, inflation-and-prices, macro-modelling-forecasting-and-data |
| 일치 점수 | monetary-policy-and-central-banking: 48, inflation-and-prices: 28, macro-modelling-forecasting-and-data: 12 |

## 원문 초록 또는 원문 선두 발췌

Assigning a discretionary central bank a mandate to stabilize an average inflation rate— rather than a period-by-period inflation rate—increases welfare in a New Keynesian model with an occasionally binding lower bound on nominal interest rates. Under rational expectations, the welfare-maximizing averaging window is infinitely long, which means that optimal aver- age inflation targeting (AIT) is equivalent to price level targeting (PLT). However, AIT with a finite, but sufficiently long, averaging window can attain most of the welfare gain from PLT. Un- der boundedly-rational expectations, if cognitive limitations are sufficiently strong, the optimal averaging window is finite, and the welfare gain of adopting AIT can be small.

## 연결

- 상위 색인: [[BIS-Fulltext-Topic-Index]]
- 구조화 노트(연구질문·방법·결과·한계): [[BIS_WP_852]]

## References

[1]: https://www.bis.org/publ/work852.pdf "BIS Working Paper 852: Average inflation targeting and the interest rate lower bound"
