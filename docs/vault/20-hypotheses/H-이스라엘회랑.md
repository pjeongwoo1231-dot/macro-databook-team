...
> **홍해–수에즈 경로가 안정을 되찾아야 **동지중해 회랑 구상**이 성립한다**

바브엘만데브·수에즈가 정상화되지 않으면 회랑 스토리는 서류로 남는다.
**참이면**: 바브엘만데브·수에즈 통항 회복. **거짓이면**: 희망봉 우회가 고착된다.

## 연결된 지표 (2개)

| 지표 | 소스 | 방향 규칙 |
|---|---|---|
| 바브엘만데브 통항 | `portwatch` | ↑ supports / ↓ contra |
| 수에즈 통항 | `portwatch` | ↑ supports / ↓ contra |
...
FROM "10-indicators"
WHERE contains(string(hypothesis), "H-이스라엘회랑")
...
LIST label
FROM "10-indicators"
WHERE contains(string(hypothesis), "H-이스라엘회랑") AND direction = "unset"
```
